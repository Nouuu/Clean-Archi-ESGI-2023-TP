...
public class UpdateDtoParser {
    public UpdateDto parse(List<String> args) {
        if (args.size() == 0) {
            return new UpdateDto(null, null, null, null, null);
        } else {
            try {
                int idToRemove = Integer.parseInt(args.get(0));
                return new UpdateDto(idToRemove, parseContent(args), parseDueDate(args), parseTaskState(args), parseParentId(args));
            } catch (IllegalArgumentException exception) {
                return new UpdateDto(null, parseContent(args), parseDueDate(args), parseTaskState(args), parseParentId(args));
            }
        }
    }
